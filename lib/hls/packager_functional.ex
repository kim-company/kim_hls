defmodule HLS.Packager.Functional do
  @moduledoc """
  ...
  ```elixir
  # 1. Initialize
  {:ok, state} = Packager.Functional.new(
    manifest_uri: URI.new!("stream.m3u8"),
    max_segments: 10
  )

  # 2. Add tracks
  {state, []} = Packager.Functional.add_track(state, "video",
  ...
  {state, [action]} = Packager.Functional.put_segment(state, "video", duration: 5.2)
  ...
  {state, actions} = Packager.Functional.sync(state, 3)
  # actions = [
  ...
  ## Examples
      ...
      # Resume
      {:ok, state} = Packager.Functional.resume(
  ...
  ## Examples
      ...
      {state, []} = Packager.Functional.add_track(state, "video_720p",
        stream: %VariantStream{
          bandwidth: 2_500_000,
          resolution: {1280, 720},
  ...
  ## Examples
      ...
      {state, [action]} = Packager.Functional.put_init_section(state, "video")
      ...
      # Confirm
      {state, []} = Packager.Functional.confirm_init_upload(state, action.id)
  """
  ...
  ## Examples
      ...
      {state, actions} = Packager.Functional.confirm_upload(state, "video_seg_1")
      # actions = [
  ...
  ## Examples
      ...
      {state, actions} = Packager.Functional.sync(state, 5)
      # actions = [
      #   %Action.WritePlaylist{type: :media, ...},
      #   %Action.WritePlaylist{type: :media, ...},
      #   %Action.WritePlaylist{type: :master, ...},
  ...
  ## Examples
      ...
      # VOD mode
      {state, actions} = Packager.Functional.flush(state)
      # actions = [
      ...
      # Sliding window mode
      {state, actions} = Packager.Functional.flush(sliding_state)
      # actions = [
