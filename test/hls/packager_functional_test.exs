defmodule HLS.PackagerFunctionalTest do
  @moduledoc """
  Test suite for HLS.Packager.Functional - a pure functional implementation of HLS packaging.
  ...
  ## Test Pattern
      ...
      # Initialize
      {:ok, state} = Packager.Functional.new(manifest_uri: uri, max_segments: 3)

      # Add segment (returns action to execute)
      {state, [action]} = Packager.Functional.put_segment(state, "video", duration: 5.0)

      # Execute action (test helper)
      execute_upload_action(action, segment_payload)

      # Confirm upload (may return playlist write actions)
      {state, actions} = Packager.Functional.confirm_upload(state, action.id)
  ...
  doctest HLS.Packager.Functional

  alias HLS.Packager.Functional
  alias HLS.Packager.Functional.Action
  ...
  defmodule ActionExecutor do
    ...
    def execute_action(%Action.UploadSegment{uri: uri}, storage, manifest_uri, payload) do
    ...
    def execute_action(%Action.UploadInitSection{uri: uri}, storage, manifest_uri, payload) do
      full_uri = HLS.Playlist.Media.build_segment_uri(manifest_uri, uri)
      TestStorage.put(storage, full_uri, payload || "test_init_payload")
    end

    def execute_action(%Action.WritePlaylist{uri: uri, content: content}, storage, manifest_uri, _payload) do
      full_uri = if uri == manifest_uri, do: uri, else: HLS.Playlist.build_absolute_uri(manifest_uri, uri)
      TestStorage.put(storage, full_uri, content)
    end

    def execute_action(%Action.DeleteSegment{uri: uri}, storage, manifest_uri, _payload) do
      full_uri = HLS.Playlist.Media.build_segment_uri(manifest_uri, uri)
      TestStorage.delete(storage, full_uri)
    end

    def execute_action(%Action.DeleteInitSection{uri: uri}, storage, manifest_uri, _payload) do
      full_uri = HLS.Playlist.Media.build_segment_uri(manifest_uri, uri)
      TestStorage.delete(storage, full_uri)
    end

    def execute_action(%Action.DeletePlaylist{uri: uri}, storage, manifest_uri, _payload) do
      full_uri = if uri == manifest_uri, do: uri, else: HLS.Playlist.build_absolute_uri(manifest_uri, uri)
      TestStorage.delete(storage, full_uri)
    end
  ...
  describe "initialization" do
    test "creates new packager state", %{manifest_uri: manifest_uri} do
      {:ok, state} = Functional.new(manifest_uri: manifest_uri, max_segments: 10)
    ...
    test "requires manifest_uri" do
      assert {:error, "manifest_uri is required"} = Functional.new(max_segments: 10)
    end
  end

  describe "track management" do
    test "adds a track successfully", %{manifest_uri: manifest_uri} do
      {:ok, state} = Functional.new(manifest_uri: manifest_uri)

      {state, []} =
        Functional.add_track(state, "video_720p",
    ...
    test "cannot add track after master is written", %{manifest_uri: manifest_uri} do
      ...
      {state, []} =
        Functional.add_track(state, "video",
      ...
      assert_raise RuntimeError, "Cannot add track after master playlist is written", fn ->
        Functional.add_track(state, "audio",
  ...
  describe "segment operations" do
    setup %{manifest_uri: manifest_uri, storage: storage} do
      ...
      {state, []} =
        Functional.add_track(state, "test_track",
          stream: %VariantStream{
    ...
    test "put_segment returns upload action", %{state: state} do
      {state, [action]} = Functional.put_segment(state, "test_track", duration: 5.5)

      assert %Action.UploadSegment{} = action
    ...
    test "confirm_upload moves segment to pending playlist", %{state: state, storage: storage, manifest_uri: manifest_uri} do
      {state, [action]} = Functional.put_segment(state, "test_track", duration: 5.5)
      ...
      {state, actions} = Functional.confirm_upload(state, action.id)

      assert length(actions) == 1
      assert [%Action.WritePlaylist{type: :pending}] = actions
    ...
    test "confirm_upload with sliding window doesn't write pending playlist", context do
      ...
      {:ok, state} = Functional.new(manifest_uri: context.manifest_uri, max_segments: 3)
    ...
    test "multiple segments can be uploaded before confirmation", %{state: state, storage: storage, manifest_uri: manifest_uri} do
      # Add 3 segments
      {state, actions} =
        Enum.reduce(1..3, {state, []}, fn i, {s, acc_actions} ->
          {new_s, actions} = Functional.put_segment(s, "test_track", duration: 6.0 + i * 0.1)
      ...
      {state, all_write_actions} =
        Enum.reduce(actions, {state, []}, fn action, {s, acc} ->
          {new_s, write_actions} = Functional.confirm_upload(s, action.id)
  ...
  describe "synchronization" do
    ...
    test "sync moves segments from pending to media playlist", %{state: state, storage: storage, manifest_uri: manifest_uri} do
      # Add and confirm 3 segments
      {state, _} =
        Enum.reduce(1..3, {state, []}, fn _i, {s, _} ->
          {s, [action]} = Functional.put_segment(s, "test_track", duration: 6.0)
          ActionExecutor.execute_action(action, storage, manifest_uri)
          {s, actions} = Functional.confirm_upload(s, action.id)
          ActionExecutor.execute_actions(actions, storage, manifest_uri)
          {s, []}
        end)
      ...
      # Sync to point 3
      {state, actions} = Functional.sync(state, 3)

      # Should have media playlist write action
      assert Enum.any?(actions, &match?(%Action.WritePlaylist{type: :media}, &1))
      ActionExecutor.execute_actions(actions, storage, manifest_uri)
    ...
    test "next_sync_point returns correct value", %{state: state, storage: storage, manifest_uri: manifest_uri} do
      assert Functional.next_sync_point(state) == 1

      # Add one segment and sync
      {state, [action]} = Functional.put_segment(state, "test_track", duration: 6.0)
      ActionExecutor.execute_action(action, storage, manifest_uri)
      {state, actions} = Functional.confirm_upload(state, action.id)
      ActionExecutor.execute_actions(actions, storage, manifest_uri)
      {state, actions} = Functional.sync(state, 1)
      ActionExecutor.execute_actions(actions, storage, manifest_uri)

      assert Functional.next_sync_point(state) == 2
    end
  end

  describe "master playlist generation" do
    test "sync writes master playlist after sync point 3", %{manifest_uri: manifest_uri, storage: storage} do
      {:ok, state} = Functional.new(manifest_uri: manifest_uri)
      ...
      {state, _} =
        Enum.reduce(1..3, {state, []}, fn _i, {s, _} ->
          {s, [action]} = Functional.put_segment(s, "video", duration: 6.0)
          ActionExecutor.execute_action(action, storage, manifest_uri)
          {s, actions} = Functional.confirm_upload(s, action.id)
          ActionExecutor.execute_actions(actions, storage, manifest_uri)
          {s, []}
      ...
      assert Enum.any?(actions, &match?(%Action.WritePlaylist{type: :master}, &1))
    ...
    test "master playlist includes alternative renditions", %{manifest_uri: manifest_uri, storage: storage} do
      {:ok, state} = Functional.new(manifest_uri: manifest_uri)
      ...
      {state, []} =
        Functional.add_track(state, "audio_en",
      ...
      state =
        Enum.reduce(["video", "audio_en"], state, fn track_id, s ->
          {s, [action]} = Functional.put_segment(s, track_id, duration: 6.0)
          ActionExecutor.execute_action(action, storage, manifest_uri)
          {s, actions} = Functional.confirm_upload(s, action.id)
          ActionExecutor.execute_actions(actions, storage, manifest_uri)
          s
      ...
      {_state, actions} = Functional.sync(state, 3)
      ActionExecutor.execute_actions(actions, storage, manifest_uri)
  ...
  describe "sliding window functionality" do
    test "sliding window limits segments in playlists", %{manifest_uri: manifest_uri, storage: storage} do
      max_segments = 3
      {:ok, state} = Functional.new(manifest_uri: manifest_uri, max_segments: max_segments)
      ...
      {state, _} =
        Enum.reduce(1..total_segments, {state, []}, fn _i, {s, _} ->
          {s, [action]} = Functional.put_segment(s, "test_track", duration: 1.0)
          ActionExecutor.execute_action(action, storage, manifest_uri)
          {s, actions} = Functional.confirm_upload(s, action.id)
          ActionExecutor.execute_actions(actions, storage, manifest_uri)
          {s, []}
      ...
      {state, actions} = Functional.sync(state, 10)

      # Should have delete actions for old segments
      delete_actions = Enum.filter(actions, &match?(%Action.DeleteSegment{}, &1))
    ...
    test "packager without max_segments keeps all segments", %{manifest_uri: manifest_uri, storage: storage} do
      {:ok, state} = Functional.new(manifest_uri: manifest_uri)
      ...
      {state, actions} = Functional.flush(state)
      ActionExecutor.execute_actions(actions, storage, manifest_uri)
    ...
    test "flush operation cleans up storage when max_segments is set", %{manifest_uri: manifest_uri, storage: storage} do
      max_segments = 2
      {:ok, state} = Functional.new(manifest_uri: manifest_uri, max_segments: max_segments)
      ...
      segment_deletes = Enum.filter(actions, &match?(%Action.DeleteSegment{}, &1))
      playlist_deletes = Enum.filter(actions, &match?(%Action.DeletePlaylist{}, &1))

      assert length(segment_deletes) > 0
      assert Enum.any?(playlist_deletes, &match?(%Action.DeletePlaylist{type: :master}, &1))
    ...
    test "flush operation creates VOD when max_segments is nil", %{manifest_uri: manifest_uri, storage: storage} do
      {:ok, state} = Functional.new(manifest_uri: manifest_uri)
      ...
      write_actions = Enum.filter(actions, &match?(%Action.WritePlaylist{}, &1))
      assert Enum.any?(write_actions, &match?(%Action.WritePlaylist{type: :media}, &1))
      assert Enum.any?(write_actions, &match?(%Action.WritePlaylist{type: :master}, &1))
    ...
    test "sliding window preserves shared init sections", %{manifest_uri: manifest_uri, storage: storage} do
      max_segments = 2
      {:ok, state} = Functional.new(manifest_uri: manifest_uri, max_segments: max_segments)
      ...
      # Set init section
      {state, [init_action]} = Functional.put_init_section(state, "test_track")
      ActionExecutor.execute_action(init_action, storage, manifest_uri, "shared_init_data")
      {state, []} = Functional.confirm_init_upload(state, init_action.id)
      ...
      segment_deletes = Enum.filter(actions, &match?(%Action.DeleteSegment{}, &1))
      init_deletes = Enum.filter(actions, &match?(%Action.DeleteInitSection{}, &1))
    ...
    test "assigns program date times to segments when max_segments is configured", %{manifest_uri: manifest_uri, storage: storage} do
      max_segments = 3
      {:ok, state} = Functional.new(manifest_uri: manifest_uri, max_segments: max_segments)
      ...
      {state, _} =
        Enum.reduce(segment_durations, {state, []}, fn duration, {s, _} ->
          {s, [action]} = Functional.put_segment(s, "test_track", duration: duration)
          ActionExecutor.execute_action(action, storage, manifest_uri)
          {s, actions} = Functional.confirm_upload(s, action.id)
          ActionExecutor.execute_actions(actions, storage, manifest_uri)
          {s, []}
    ...
    test "does not assign program date times when max_segments is nil", %{manifest_uri: manifest_uri, storage: storage} do
      {:ok, state} = Functional.new(manifest_uri: manifest_uri)
      ...
      {state, _} =
        Enum.reduce(1..3, {state, []}, fn _i, {s, _} ->
          {s, [action]} = Functional.put_segment(s, "test_track", duration: 2.0)
          ActionExecutor.execute_action(action, storage, manifest_uri)
          {s, actions} = Functional.confirm_upload(s, action.id)
          ActionExecutor.execute_actions(actions, storage, manifest_uri)
          {s, []}
  ...
  describe "RFC 8216 compliance" do
    ...
    test "playlists are UTF-8 encoded without BOM", %{manifest_uri: manifest_uri, storage: storage} do
      {:ok, state} = Functional.new(manifest_uri: manifest_uri)
      ...
      {state, [action]} = Functional.put_segment(state, "video", duration: 6.0)
      ActionExecutor.execute_action(action, storage, manifest_uri)
      {state, actions} = Functional.confirm_upload(state, action.id)
      ActionExecutor.execute_actions(actions, storage, manifest_uri)
      {_state, actions} = Functional.sync(state, 3)
    ...
    test "target duration remains constant", %{manifest_uri: manifest_uri, storage: storage} do
      ...
      {state, _media_contents} =
        Enum.reduce(durations, {state, []}, fn duration, {s, contents} ->
          {s, [action]} = Functional.put_segment(s, "video", duration: duration)
          ActionExecutor.execute_action(action, storage, manifest_uri)
          {s, actions} = Functional.confirm_upload(s, action.id)
          ActionExecutor.execute_actions(actions, storage, manifest_uri)
          ...
          media_uri = HLS.Playlist.build_absolute_uri(manifest_uri, URI.new!("stream_video.m3u8"))
          content = case TestStorage.get(storage, media_uri) do
            {:ok, c} -> c
            {:error, :not_found} -> nil
          end
      ...
      {_state, actions} = Functional.sync(state, 10)
      ActionExecutor.execute_actions(actions, storage, manifest_uri)
    ...
    test "segment durations do not exceed target duration", %{manifest_uri: manifest_uri, storage: storage} do
      {:ok, state} = Functional.new(manifest_uri: manifest_uri)
    ...
    test "media sequence numbers are monotonically increasing", %{manifest_uri: manifest_uri, storage: storage} do
      max_segments = 3
      {:ok, state} = Functional.new(manifest_uri: manifest_uri, max_segments: max_segments)
      ...
      {_state, media_sequences} =
        Enum.reduce(1..10, {state, media_sequences}, fn _i, {s, seqs} ->
          {s, [action]} = Functional.put_segment(s, "video", duration: 2.0)
          ActionExecutor.execute_action(action, storage, manifest_uri)
          {s, actions} = Functional.confirm_upload(s, action.id)
          ActionExecutor.execute_actions(actions, storage, manifest_uri)
          {s, actions} = Functional.sync(s, Functional.next_sync_point(s))
          ActionExecutor.execute_actions(actions, storage, manifest_uri)
    ...
    test "discontinuity sequence increments properly", %{manifest_uri: manifest_uri, storage: storage} do
      max_segments = 3
      {:ok, state} = Functional.new(manifest_uri: manifest_uri, max_segments: max_segments)
      ...
      # Add segment
      {state, [action]} = Functional.put_segment(state, "video", duration: 2.0)
      ActionExecutor.execute_action(action, storage, manifest_uri)
      {state, actions} = Functional.confirm_upload(state, action.id)
      ActionExecutor.execute_actions(actions, storage, manifest_uri)
      {state, actions} = Functional.sync(state, 1)
      ...
      # Add discontinuity
      {state, []} = Functional.discontinue(state)
      ...
      {state, actions} = Functional.confirm_upload(state, action.id)
      ActionExecutor.execute_actions(actions, storage, manifest_uri)
      {state, actions} = Functional.sync(state, 2)
      ActionExecutor.execute_actions(actions, storage, manifest_uri)
    ...
    test "program date time assigned to discontinuity segments", %{manifest_uri: manifest_uri, storage: storage} do
      max_segments = 5
      {:ok, state} = Functional.new(manifest_uri: manifest_uri, max_segments: max_segments)
    ...
    test "EXT-X-ENDLIST only appears in VOD playlists", %{manifest_uri: manifest_uri, storage: storage} do
      # Test live playlist (max_segments set) - should NOT have ENDLIST
      max_segments = 3
      {:ok, live_state} = Functional.new(manifest_uri: manifest_uri, max_segments: max_segments)

      {live_state, []} =
        Functional.add_track(live_state, "video",
          stream: %VariantStream{bandwidth: 1_000_000, codecs: []},
          segment_extension: ".ts",
          target_segment_duration: 2.0
        )

      {live_state, [action]} = Functional.put_segment(live_state, "video", duration: 2.0)
      ActionExecutor.execute_action(action, storage, manifest_uri)
      {live_state, actions} = Functional.confirm_upload(live_state, action.id)
      ActionExecutor.execute_actions(actions, storage, manifest_uri)
      {_live_state, actions} = Functional.sync(live_state, 1)
      ActionExecutor.execute_actions(actions, storage, manifest_uri)
      ...
      {:ok, vod_state} = Functional.new(manifest_uri: manifest_uri)

      {vod_state, []} =
        Functional.add_track(vod_state, "video",
          stream: %VariantStream{bandwidth: 1_000_000, codecs: []},
          segment_extension: ".ts",
          target_segment_duration: 2.0
        )

      {vod_state, [action]} = Functional.put_segment(vod_state, "video", duration: 2.0)
      ActionExecutor.execute_action(action, storage, manifest_uri)
      {vod_state, actions} = Functional.confirm_upload(vod_state, action.id)
      ActionExecutor.execute_actions(actions, storage, manifest_uri)
      {_vod_state, actions} = Functional.flush(vod_state)
      ActionExecutor.execute_actions(actions, storage, manifest_uri)
    ...
    test "matching timestamps across variant streams", %{manifest_uri: manifest_uri, storage: storage} do
      ...
      {state, []} =
        Functional.add_track(state, "video_high",
          stream: %VariantStream{
            bandwidth: 2_000_000,
            resolution: {1280, 720},
            codecs: []
      ...
      {state, []} =
        Functional.add_track(state, "video_low",
          stream: %VariantStream{
            bandwidth: 500_000,
            resolution: {640, 360},
            codecs: []
      ...
      {state, _} =
        Enum.reduce(1..4, {state, []}, fn _i, {s, _} ->
          # Add to both tracks
          {s, [action1]} = Functional.put_segment(s, "video_high", duration: 2.0)
          ActionExecutor.execute_action(action1, storage, manifest_uri)
          {s, actions1} = Functional.confirm_upload(s, action1.id)
          ActionExecutor.execute_actions(actions1, storage, manifest_uri)

          {s, [action2]} = Functional.put_segment(s, "video_low", duration: 2.0)
          ActionExecutor.execute_action(action2, storage, manifest_uri)
          {s, actions2} = Functional.confirm_upload(s, action2.id)
    ...
    test "matching discontinuity sequences across variant streams", %{manifest_uri: manifest_uri, storage: storage} do
      ...
      state =
        Enum.reduce(["video", "audio"], state, fn track_id, s ->
          {s, [action]} = Functional.put_segment(s, track_id, duration: 2.0)
          ActionExecutor.execute_action(action, storage, manifest_uri)
          {s, actions} = Functional.confirm_upload(s, action.id)
          ActionExecutor.execute_actions(actions, storage, manifest_uri)
          s
        ...
        Enum.reduce(1..4, {state, []}, fn _i, {s, _} ->
          s =
            Enum.reduce(["video", "audio"], s, fn track_id, inner_s ->
              {inner_s, [action]} = Functional.put_segment(inner_s, track_id, duration: 2.0)
              ActionExecutor.execute_action(action, storage, manifest_uri)
              {inner_s, actions} = Functional.confirm_upload(inner_s, action.id)
              ActionExecutor.execute_actions(actions, storage, manifest_uri)
              inner_s
            end)
